...
class CastCertificateController extends GetxController {
  bool loader = false;
  ...
  void onInit() {
    super.onInit();
    getCastCiertificateData();
  }


  //Call Cast Ciertificate Api
  Future<void> getCastCiertificateData() async {

    if (await isInternetAvailable(context)) {
      FocusManager.instance.primaryFocus?.unfocus();
      loader = true;
      update(['cast_certificate_screen']);

      try {
        CastCertificateModel? model = await CastCertificateApi.getCastCiertificateData();


//        loadNetwork("https://icseindia.org/document/sample.pdf");
        if (model != null) {
  ...
  Future<void> saveFile() async {
    ...
    String fileName="/storage/emulated/0/${PrefKeys.JAN_DOC_PATH}-${PrefService.getString(PrefKeys.FamilyID)}/"+PrefKeys.DOC_SARAL_CASTE+PrefService.getString(PrefKeys.FamilyID)+".pdf";
