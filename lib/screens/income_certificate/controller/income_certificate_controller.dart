...
class IncomeCertificateController extends GetxController {
  bool loader = false;
  ...
  void onInit() {
    super.onInit();
    getIncomeCiertificateData();
  }


  //Call Income Ciertificate Api
  Future<void> getIncomeCiertificateData() async {

    if (await isInternetAvailable(context)) {
      FocusManager.instance.primaryFocus?.unfocus();
      loader = true;
      update(['income_certificate_screen']);

      try {
        CastCertificateModel? model = await IncomeCertificateApi.getIncomeCiertificateData();
  ...
  Future<void> saveFile() async {
    ...
    String fileName="/storage/emulated/0/${PrefKeys.JAN_DOC_PATH}-${PrefService.getString(PrefKeys.FamilyID)}/"+PrefKeys.DOC_SARAL_INCOME+PrefService.getString(PrefKeys.FamilyID)+".pdf";
